...
#Region "Private properties"
    Private _SelectData As New UserUIInfo
#End Region

#Region "【プロパティ】 選択情報 "
    Public ReadOnly Property SelectData() As UserUIInfo
        Get
            Return _SelectData
        End Get
    End Property
...
#Region "【メソッド】 "
    Private Function GetDataFromScreen() As UserUIFilterInfo
        Try
            Dim chumonSearchInfo As New UserUIFilterInfo
            ...
            If String.IsNullOrEmpty(txtEmail.Text) = False Then chumonSearchInfo.Email = txtEmail.Text
            If String.IsNullOrEmpty(txtUserName.Text) = False Then chumonSearchInfo.UserName = txtUserName.Text
            If String.IsNullOrEmpty(txtLanguage.Text) = False Then chumonSearchInfo.Language = txtLanguage.Text
            If String.IsNullOrEmpty(txtPassword.Text) = False Then chumonSearchInfo.Password = txtPassword.Text
            If String.IsNullOrEmpty(txtPasswordHash.Text) = False Then chumonSearchInfo.PasswordHash = txtPasswordHash.Text
            If String.IsNullOrEmpty(numRoleId.Text) = False Then chumonSearchInfo.RoleId = numRoleId.Text
...
#Region "【メソッド】 SearchBtnClick"
    Private Sub SearchBtnClick()
        ...
        Dim cls As New UserUIFilterInfo
        Dim lst As New List(Of UserUIInfo)
        ...
        Try
            ...
            With MseDataGrid
                ...
                For Each row As UserUIInfo In lst
                    .Rows.Add()
                    .Rows(MseDataGrid.RowCount - 1).Tag = row
                    .Rows(MseDataGrid.RowCount - 1).Cells(USER_COL.ID).Value = row.Id
                    .Rows(MseDataGrid.RowCount - 1).Cells(USER_COL.CODE).Value = row.Code
                    .Rows(MseDataGrid.RowCount - 1).Cells(USER_COL.NAME).Value = row.Name
                    .Rows(MseDataGrid.RowCount - 1).Cells(USER_COL.EMAIL).Value = row.Email
                    .Rows(MseDataGrid.RowCount - 1).Cells(USER_COL.USER_NAME).Value = row.UserName
                    .Rows(MseDataGrid.RowCount - 1).Cells(USER_COL.LANGUAGE).Value = row.Language
                    .Rows(MseDataGrid.RowCount - 1).Cells(USER_COL.PASSWORD).Value = row.Password
                    .Rows(MseDataGrid.RowCount - 1).Cells(USER_COL.PASSWORD_HASH).Value = row.PasswordHash
                    .Rows(MseDataGrid.RowCount - 1).Cells(USER_COL.ROLE_ID).Value = row.RoleId
                    .Rows(MseDataGrid.RowCount - 1).Cells(USER_COL.CREATED_AT).Value = row.CreatedAt
                    .Rows(MseDataGrid.RowCount - 1).Cells(USER_COL.UPDATED_AT).Value = row.UpdatedAt
                    .Rows(MseDataGrid.RowCount - 1).Cells(USER_COL.YUKO_FLAG).Value = row.YukoFlag
                    .Rows(MseDataGrid.RowCount - 1).Cells(USER_COL.CREATED_USER).Value = row.CreatedUser
                    .Rows(MseDataGrid.RowCount - 1).Cells(USER_COL.LAST_UPDATE_USER).Value = row.LastUpdateUser
                    .Rows(MseDataGrid.RowCount - 1).Cells(USER_COL.LAST_UPDATE_PROGRAM).Value = row.LastUpdateProgram
                Next
        ...
        With MseDataGrid
            If (.SelectedRows.Count > 0) Then
                ...
                If (Not IsNothing(.Rows(.SelectedRows(0).Index).Tag)) Then
                    Me._SelectData = CType(.Rows(.SelectedRows(0).Index).Tag, UserUIInfo)
                End If
...
#Region "【Event】 UserSearch_KeyDown"
    Private Sub UserSearch_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        Try
        ...
        Try
            If (e.RowIndex >= 0) Then
                If (Not IsNothing(MseDataGrid.Rows(e.RowIndex).Tag)) Then
                    Me._SelectData = CType(MseDataGrid.Rows(e.RowIndex).Tag, UserUIInfo)
                End If
            End If
        Catch ex As Exception
            NLogHelper.LogError(ex.ToString)
