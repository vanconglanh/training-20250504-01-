...
#Region "Private properties"
    Private _SelectData As New PermissionUIInfo
#End Region

#Region "【プロパティ】 選択情報 "
    Public ReadOnly Property SelectData() As PermissionUIInfo
        Get
            Return _SelectData
        End Get
    End Property
...
#Region "【メソッド】 "
    Private Function GetDataFromScreen() As PermissionUIFilterInfo
        Try
            Dim chumonSearchInfo As New PermissionUIFilterInfo
...
#Region "【メソッド】 SearchBtnClick"
    Private Sub SearchBtnClick()
        ...
        Dim cls As New PermissionUIFilterInfo
        Dim lst As New List(Of PermissionUIInfo)
        ...
        Try
            ...
            With MseDataGrid
                ...
                For Each row As PermissionUIInfo In lst
                    .Rows.Add()
                    .Rows(MseDataGrid.RowCount - 1).Tag = row
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_COL.ID).Value = row.Id
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_COL.CODE).Value = row.Code
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_COL.NAME).Value = row.Name
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_COL.CREATED_AT).Value = row.CreatedAt
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_COL.UPDATED_AT).Value = row.UpdatedAt
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_COL.YUKO_FLAG).Value = row.YukoFlag
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_COL.CREATED_USER).Value = row.CreatedUser
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_COL.LAST_UPDATE_USER).Value = row.LastUpdateUser
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_COL.LAST_UPDATE_PROGRAM).Value = row.LastUpdateProgram
                Next
        ...
        With MseDataGrid
            If (.SelectedRows.Count > 0) Then
                ...
                If (Not IsNothing(.Rows(.SelectedRows(0).Index).Tag)) Then
                    Me._SelectData = CType(.Rows(.SelectedRows(0).Index).Tag, PermissionUIInfo)
                End If
...
#Region "【Event】 PermissionSearch_Load"
    Private Sub PermissionSearch_Load(sender As Object, e As EventArgs) Handles MyBase.Load
...
#Region "【Event】 PermissionSearch_KeyDown"
    Private Sub PermissionSearch_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        Try
        ...
        Try
            If (e.RowIndex >= 0) Then
                If (Not IsNothing(MseDataGrid.Rows(e.RowIndex).Tag)) Then
                    Me._SelectData = CType(MseDataGrid.Rows(e.RowIndex).Tag, PermissionUIInfo)
                End If
            End If
        Catch ex As Exception
            NLogHelper.LogError(ex.ToString)
