...
#Region "Private properties"
    Private _SelectData As New RolePermissionUIInfo
#End Region

#Region "【プロパティ】 選択情報 "
    Public ReadOnly Property SelectData() As RolePermissionUIInfo
        Get
            Return _SelectData
        End Get
    End Property
...
#Region "【メソッド】 "
    Private Function GetDataFromScreen() As RolePermissionUIFilterInfo
        Try
            Dim chumonSearchInfo As New RolePermissionUIFilterInfo
            If String.IsNullOrEmpty(numRoleId.Text) = False Then chumonSearchInfo.RoleId = numRoleId.Text
            If String.IsNullOrEmpty(numPermissionScreenId.Text) = False Then chumonSearchInfo.PermissionScreenId = numPermissionScreenId.Text
...
#Region "【メソッド】 SearchBtnClick"
    Private Sub SearchBtnClick()
        ...
        Dim cls As New RolePermissionUIFilterInfo
        Dim lst As New List(Of RolePermissionUIInfo)
        ...
        Try
            ...
            With MseDataGrid
                ...
                For Each row As RolePermissionUIInfo In lst
                    .Rows.Add()
                    .Rows(MseDataGrid.RowCount - 1).Tag = row
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_PERMISSION_COL.ROLE_ID).Value = row.RoleId
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_PERMISSION_COL.PERMISSION_SCREEN_ID).Value = row.PermissionScreenId
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_PERMISSION_COL.CODE).Value = row.Code
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_PERMISSION_COL.CREATED_AT).Value = row.CreatedAt
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_PERMISSION_COL.UPDATED_AT).Value = row.UpdatedAt
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_PERMISSION_COL.YUKO_FLAG).Value = row.YukoFlag
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_PERMISSION_COL.CREATED_USER).Value = row.CreatedUser
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_PERMISSION_COL.LAST_UPDATE_USER).Value = row.LastUpdateUser
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_PERMISSION_COL.LAST_UPDATE_PROGRAM).Value = row.LastUpdateProgram
                Next
        ...
        With MseDataGrid
            If (.SelectedRows.Count > 0) Then
                ...
                If (Not IsNothing(.Rows(.SelectedRows(0).Index).Tag)) Then
                    Me._SelectData = CType(.Rows(.SelectedRows(0).Index).Tag, RolePermissionUIInfo)
                End If
...
#Region "【Event】 RolePermissionSearch_Load"
    Private Sub RolePermissionSearch_Load(sender As Object, e As EventArgs) Handles MyBase.Load
...
#Region "【Event】 RolePermissionSearch_KeyDown"
    Private Sub RolePermissionSearch_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        Try
        ...
        Try
            If (e.RowIndex >= 0) Then
                If (Not IsNothing(MseDataGrid.Rows(e.RowIndex).Tag)) Then
                    Me._SelectData = CType(MseDataGrid.Rows(e.RowIndex).Tag, RolePermissionUIInfo)
                End If
            End If
        Catch ex As Exception
            NLogHelper.LogError(ex.ToString)
