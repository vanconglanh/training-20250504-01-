...
#Region "Private変数"
    ...
    Private _ScreenSearchForm As New ScreenSearch()
    Private _screenUIInfo As New ScreenUIInfo
...
#Region "WndProc"
    Protected Overrides Sub WndProc(ByRef m As Message)
        ...
        If m.Msg = WM_CLOSE Then
            'TODO
            ' If Me.ActiveControl Is txtScreenNo Or
...
#Region "【メソッド】 GeyDataByKey"
    Private Function GeyDataByKey() As ScreenUIInfo
        Dim business As New ScreenEntryBusiness
        Dim cls As New ScreenUIInfo
...
#Region "【メソッド】 CheckChanged"
    Private Function CheckChanged() As Boolean
        If ( 
                Me._screenUIInfo.Id <> CDbl(IIF(String.IsNullOrEmpty(numId.Text),0,numId.Text))  OrElse
                Me._screenUIInfo.Code <> txtCode.Text  OrElse
                Me._screenUIInfo.Name <> txtName.Text  OrElse
                (Me._screenUIInfo.CreatedAt Is Nothing AndAlso dteCreatedAt.Text <> "") OrElse (Me._ScreenUIInfo.CreatedAt IsNot Nothing AndAlso Me._ScreenUIInfo.CreatedAt <> dteCreatedAt.Text)  OrElse
                (Me._screenUIInfo.UpdatedAt Is Nothing AndAlso dteUpdatedAt.Text <> "") OrElse (Me._ScreenUIInfo.UpdatedAt IsNot Nothing AndAlso Me._ScreenUIInfo.UpdatedAt <> dteUpdatedAt.Text)  OrElse
                Me._screenUIInfo.YukoFlag <> txtYukoFlag.Text  OrElse
                Me._screenUIInfo.CreatedUser <> CDbl(IIF(String.IsNullOrEmpty(numCreatedUser.Text),0,numCreatedUser.Text))  OrElse
                Me._screenUIInfo.LastUpdateUser <> CDbl(IIF(String.IsNullOrEmpty(numLastUpdateUser.Text),0,numLastUpdateUser.Text))  OrElse
                Me._screenUIInfo.LastUpdateProgram <> txtLastUpdateProgram.Text  OrElse 1 <> 0) Then
            Return True
        End If
...
#Region "【メソッド】 ScreenEntry_Load"
    Private Sub ScreenEntry_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            '---
            Me._Mode = MODE.INIT
...
#Region "【メソッド】 ScreenEntry_Closing"
    Private Sub ScreenEntry_Closing(sender As Object, e As FormClosingEventArgs) Handles MyBase.Closing
    ...
    Private Sub numId_KeyDown(sender As Object, e As KeyEventArgs) Handles numId.KeyDown
        ...
        Try
            If e.KeyCode = Keys.F3 Then             'F3: Event
                '--- Show search dialog
                Me._ScreenSearchForm.ShowDialog()

                If (Not IsNothing(Me._ScreenSearchForm.SelectData)) Then
                    '--- Clear current data
                    Init()

                    '--- Set code
                    numId.Text = CType(Me._ScreenSearchForm.SelectData, ScreenUIInfo).Id
...
#Region "【メソッド】 numScreenId_Validating"
...
#Region "【メソッド】 ScreenEntry_KeyDown"
    Private Sub ScreenEntry_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        Try
...
#Region "Set EntryInfo to control"
    Private Sub SetEntryInfoToControl(ByVal info As ScreenUIInfo)
