...
Public Class PermissionScreenEntry
...
#Region "Private変数"
    ...
    Private _PermissionScreenSearchForm As New PermissionScreenSearch()
    Private _permissionScreenUIInfo As New PermissionScreenUIInfo
...
#Region "WndProc"
    Protected Overrides Sub WndProc(ByRef m As Message)
        ...
        If m.Msg = WM_CLOSE Then
            'TODO
            ' If Me.ActiveControl Is txtPermissionScreenNo Or
    ...
    Private Sub Init(Optional ByVal vboolModeNewFlag As Boolean = False)
        Try
            ...
            With MseDataGridRolePermission
                ...
                .Columns(ROLE_PERMISSION_COL.ID).Visible = False
                .Columns(ROLE_PERMISSION_COL.ROLE_ID, PERMISSION_SCREEN_ID).Visible = False
            End With
    ...
    Private Function CheckItems(ByVal Optional vBoolDelFlag As Boolean = False) As Boolean
        ...
        Try
            ...
            If (Not vBoolDelFlag) Then
                ...
                '--- PermissionId
                If String.IsNullOrEmpty(numPermissionId.Text) Then   
                    MessageBox.Show(String.Format(My.Resources.ERR_RequiredCheck, numPermissionId.MsgTitle),
                                    My.Resources.UI_Error,
                                    MessageBoxButtons.OK,
                                    MessageBoxIcon.Error)
                    numPermissionId.Focus()
                    Return False
                End If      

                '--- ScreenId
                If String.IsNullOrEmpty(numScreenId.Text) Then   
                    MessageBox.Show(String.Format(My.Resources.ERR_RequiredCheck, numScreenId.MsgTitle),
                                    My.Resources.UI_Error,
                                    MessageBoxButtons.OK,
                                    MessageBoxIcon.Error)
                    numScreenId.Focus()
...
#Region "【メソッド】 CheckDeletedByKey"
    Private Function CheckDeletedByKey() As Boolean
        Dim business As New PermissionScreenSearchBusiness
        Dim bResult As Boolean = True
...
#Region "【メソッド】 GeyDataByKey"
    Private Function GeyDataByKey() As PermissionScreenUIInfo
        Dim business As New PermissionScreenEntryBusiness
        Dim cls As New PermissionScreenUIInfo
...
#Region "【メソッド】 CheckChanged"
    Private Function CheckChanged() As Boolean
        If ( 
                Me._permissionScreenUIInfo.Id <> CDbl(IIF(String.IsNullOrEmpty(numId.Text),0,numId.Text))  OrElse
                Me._permissionScreenUIInfo.PermissionId <> CDbl(IIF(String.IsNullOrEmpty(numPermissionId.Text),0,numPermissionId.Text))  OrElse
                Me._permissionScreenUIInfo.ScreenId <> CDbl(IIF(String.IsNullOrEmpty(numScreenId.Text),0,numScreenId.Text))  OrElse
                Me._permissionScreenUIInfo.Code <> txtCode.Text  OrElse
                (Me._permissionScreenUIInfo.CreatedAt Is Nothing AndAlso dteCreatedAt.Text <> "") OrElse (Me._PermissionScreenUIInfo.CreatedAt IsNot Nothing AndAlso Me._PermissionScreenUIInfo.CreatedAt <> dteCreatedAt.Text)  OrElse
                (Me._permissionScreenUIInfo.UpdatedAt Is Nothing AndAlso dteUpdatedAt.Text <> "") OrElse (Me._PermissionScreenUIInfo.UpdatedAt IsNot Nothing AndAlso Me._PermissionScreenUIInfo.UpdatedAt <> dteUpdatedAt.Text)  OrElse
                Me._permissionScreenUIInfo.YukoFlag <> txtYukoFlag.Text  OrElse
                Me._permissionScreenUIInfo.CreatedUser <> CDbl(IIF(String.IsNullOrEmpty(numCreatedUser.Text),0,numCreatedUser.Text))  OrElse
                Me._permissionScreenUIInfo.LastUpdateUser <> CDbl(IIF(String.IsNullOrEmpty(numLastUpdateUser.Text),0,numLastUpdateUser.Text))  OrElse
                Me._permissionScreenUIInfo.LastUpdateProgram <> txtLastUpdateProgram.Text  OrElse 1 <> 0) Then
            Return True
        End If
...
#Region "【メソッド】 SaveBtnClick"
    Private Sub SaveBtnClick()
        ...
        Dim cls As New PermissionScreenUIInfo
        Dim rolePermissionUIInfo As RolePermissionUIInfo
        ...
        With cls
            .Id = numId.Text
            .PermissionId = numPermissionId.Text
            .ScreenId = numScreenId.Text
            .Code = txtCode.Text
        ...
        For iRow = 0 To MseDataGridPermissionScreen.Rows.Count - 2
            ...
            With permissionScreenUIInfo
                    ...
                    .RoleId = MseDataGridPermissionScreen.Rows(iRow).Cells(PERMISSION_SCREEN_COL.ROLE_ID).Value
                    .PermissionScreenId = MseDataGridPermissionScreen.Rows(iRow).Cells(PERMISSION_SCREEN_COL.PERMISSION_SCREEN_ID).Value
        ...
        Me._permissionScreenUIInfo = cls
...
#Region "【メソッド】 PermissionScreenEntry_Load"
    Private Sub PermissionScreenEntry_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            '---
            Me._Mode = MODE.INIT
...
#Region "【メソッド】 PermissionScreenEntry_Closing"
    Private Sub PermissionScreenEntry_Closing(sender As Object, e As FormClosingEventArgs) Handles MyBase.Closing
        ...
        Try
            If e.CloseReason = CloseReason.UserClosing Then
                ...
                Me._PermissionScreenSearchForm.Dispose()
            End If
    ...
    Private Sub numId_KeyDown(sender As Object, e As KeyEventArgs) Handles numId.KeyDown
        ...
        Try
            If e.KeyCode = Keys.F3 Then             'F3: Event
                '--- Show search dialog
                Me._PermissionScreenSearchForm.ShowDialog()

                If (Not IsNothing(Me._PermissionScreenSearchForm.SelectData)) Then
                    '--- Clear current data
                    Init()

                    '--- Set code
                    numId.Text = CType(Me._PermissionScreenSearchForm.SelectData, PermissionScreenUIInfo).Id
...
#Region "【メソッド】 numPermissionScreenId_Validating"
...
#Region "【メソッド】 PermissionScreenEntry_KeyDown"
    Private Sub PermissionScreenEntry_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        Try
...
#Region "Set EntryInfo to control"
    Private Sub SetEntryInfoToControl(ByVal info As PermissionScreenUIInfo)
        ...
        numId.Text = info.Id
        numPermissionId.Text = info.PermissionId
        numScreenId.Text = info.ScreenId
        ...
        With MseDataGridPermissionScreen
            ...
            For Each permissionScreen In info.PermissionScreenList
                ...
                .Rows(.RowCount - 2).Cells(PERMISSION_SCREEN_COL.ROLE_ID).Value = permissionScreen.RoleId
                .Rows(.RowCount - 2).Cells(PERMISSION_SCREEN_COL.PERMISSION_SCREEN_ID).Value = permissionScreen.PermissionScreenId
