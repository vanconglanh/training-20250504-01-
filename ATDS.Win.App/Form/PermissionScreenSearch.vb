...
Public Class PermissionScreenSearch
...
#Region "Private properties"
    Private _SelectData As New PermissionScreenUIInfo
#End Region

#Region "【プロパティ】 選択情報 "
    Public ReadOnly Property SelectData() As PermissionScreenUIInfo
        Get
            Return _SelectData
        End Get
    End Property
...
#Region "【メソッド】 "
    Private Function GetDataFromScreen() As PermissionScreenUIFilterInfo
        Try
            Dim chumonSearchInfo As New PermissionScreenUIFilterInfo
            If String.IsNullOrEmpty(numId.Text) = False Then chumonSearchInfo.Id = numId.Text
            If String.IsNullOrEmpty(numPermissionId.Text) = False Then chumonSearchInfo.PermissionId = numPermissionId.Text
            If String.IsNullOrEmpty(numScreenId.Text) = False Then chumonSearchInfo.ScreenId = numScreenId.Text
...
#Region "【メソッド】 SearchBtnClick"
    Private Sub SearchBtnClick()
        ...
        Dim cls As New PermissionScreenUIFilterInfo
        Dim lst As New List(Of PermissionScreenUIInfo)
        ...
        Try
            ...
            With MseDataGrid
                ...
                For Each row As PermissionScreenUIInfo In lst
                    .Rows.Add()
                    .Rows(MseDataGrid.RowCount - 1).Tag = row
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_SCREEN_COL.ID).Value = row.Id
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_SCREEN_COL.PERMISSION_ID).Value = row.PermissionId
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_SCREEN_COL.SCREEN_ID).Value = row.ScreenId
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_SCREEN_COL.CODE).Value = row.Code
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_SCREEN_COL.CREATED_AT).Value = row.CreatedAt
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_SCREEN_COL.UPDATED_AT).Value = row.UpdatedAt
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_SCREEN_COL.YUKO_FLAG).Value = row.YukoFlag
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_SCREEN_COL.CREATED_USER).Value = row.CreatedUser
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_SCREEN_COL.LAST_UPDATE_USER).Value = row.LastUpdateUser
                    .Rows(MseDataGrid.RowCount - 1).Cells(PERMISSION_SCREEN_COL.LAST_UPDATE_PROGRAM).Value = row.LastUpdateProgram
                Next
        ...
        With MseDataGrid
            If (.SelectedRows.Count > 0) Then
                ...
                If (Not IsNothing(.Rows(.SelectedRows(0).Index).Tag)) Then
                    Me._SelectData = CType(.Rows(.SelectedRows(0).Index).Tag, PermissionScreenUIInfo)
                End If
...
#Region "【Event】 PermissionScreenSearch_Load"
    Private Sub PermissionScreenSearch_Load(sender As Object, e As EventArgs) Handles MyBase.Load
...
#Region "【Event】 PermissionScreenSearch_KeyDown"
    Private Sub PermissionScreenSearch_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        Try
        ...
        Try
            If (e.RowIndex >= 0) Then
                If (Not IsNothing(MseDataGrid.Rows(e.RowIndex).Tag)) Then
                    Me._SelectData = CType(MseDataGrid.Rows(e.RowIndex).Tag, PermissionScreenUIInfo)
                End If
            End If
        Catch ex As Exception
            NLogHelper.LogError(ex.ToString)
