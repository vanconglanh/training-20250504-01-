...
#Region "Private properties"
    Private _SelectData As New RoleUIInfo
#End Region

#Region "【プロパティ】 選択情報 "
    Public ReadOnly Property SelectData() As RoleUIInfo
        Get
            Return _SelectData
        End Get
    End Property
...
#Region "【メソッド】 "
    Private Function GetDataFromScreen() As RoleUIFilterInfo
        Try
            Dim chumonSearchInfo As New RoleUIFilterInfo
            ...
            If String.IsNullOrEmpty(numIsSystem.Text) = False Then chumonSearchInfo.IsSystem = numIsSystem.Text
...
#Region "【メソッド】 SearchBtnClick"
    Private Sub SearchBtnClick()
        ...
        Dim cls As New RoleUIFilterInfo
        Dim lst As New List(Of RoleUIInfo)
        ...
        Try
            ...
            With MseDataGrid
                ...
                For Each row As RoleUIInfo In lst
                    .Rows.Add()
                    .Rows(MseDataGrid.RowCount - 1).Tag = row
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_COL.ID).Value = row.Id
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_COL.CODE).Value = row.Code
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_COL.NAME).Value = row.Name
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_COL.IS_SYSTEM).Value = row.IsSystem
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_COL.CREATED_AT).Value = row.CreatedAt
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_COL.UPDATED_AT).Value = row.UpdatedAt
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_COL.YUKO_FLAG).Value = row.YukoFlag
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_COL.CREATED_USER).Value = row.CreatedUser
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_COL.LAST_UPDATE_USER).Value = row.LastUpdateUser
                    .Rows(MseDataGrid.RowCount - 1).Cells(ROLE_COL.LAST_UPDATE_PROGRAM).Value = row.LastUpdateProgram
                Next
        ...
        With MseDataGrid
            If (.SelectedRows.Count > 0) Then
                ...
                If (Not IsNothing(.Rows(.SelectedRows(0).Index).Tag)) Then
                    Me._SelectData = CType(.Rows(.SelectedRows(0).Index).Tag, RoleUIInfo)
                End If
...
#Region "【Event】 RoleSearch_Load"
    Private Sub RoleSearch_Load(sender As Object, e As EventArgs) Handles MyBase.Load
...
#Region "【Event】 RoleSearch_KeyDown"
    Private Sub RoleSearch_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        Try
        ...
        Try
            If (e.RowIndex >= 0) Then
                If (Not IsNothing(MseDataGrid.Rows(e.RowIndex).Tag)) Then
                    Me._SelectData = CType(MseDataGrid.Rows(e.RowIndex).Tag, RoleUIInfo)
                End If
            End If
        Catch ex As Exception
            NLogHelper.LogError(ex.ToString)
