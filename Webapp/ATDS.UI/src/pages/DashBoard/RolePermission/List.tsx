...
import Filter from '@/pages/DashBoard/RolePermission/Filter';
...
import { RolePermission, RolePermissionListResponse, RolePermissionParams } from '@/types/rolePermission.type';
...
interface ColumnConfig {
  id: keyof RolePermission | 'actions';
  label: string;
  visible: boolean;
  sortable?: boolean;
}
...
const isValidData = (data: RolePermissionListResponse | undefined): data is RolePermissionListResponse => {
...
const RolePermissionList: React.FC = () => {
  ...
  const getInitialPage = () => {
    ...
    if (pageFromUrl) {
      ...
      localStorage.setItem('rolePermissionsCurrentPage', page.toString());
      return page;
    }
    return Number(localStorage.getItem('rolePermissionsCurrentPage')) || 1;
  };

  // Pagination and other states
  const [params, setParams] = useState<RolePermissionParams>({
    page: getInitialPage(),
    size: 10,
    search: [],
  });
  ...
  const [columns, setColumns] = useState<ColumnConfig[]>(() => {
    ...
    return [
      { id: 'id', label: t('rolePermissions.fields.id'), visible: true, sortable: true },
      { id: 'roleId', label: t('rolePermissions.fields.roleId'), visible: true, sortable: true },
      { id: 'permissionScreenId', label: t('rolePermissions.fields.permissionScreenId'), visible: true, sortable: true },
  ...
  const [cachedData, setCachedData] = useState<Record<string, RolePermission[]>>({});
  const [totalItems, setTotalItems] = useState(0);

  // State ﾄ黛ｻ・lﾆｰu tr盻ｯ d盻ｯ li盻㎡ hi盻ハ th盻・hi盻㌻ t蘯｡i
  const [displayData, setDisplayData] = useState<RolePermission[]>([]);
  ...
  const { data, isLoading, isFetching, refetch } = useQuery<RolePermissionListResponse>({
    queryKey: ['rolePermissions', params],
    queryFn: async () => {
      try {
        const result = await rolePermissionService.getRolePermissions(params);
        return result;
      } catch (error) {
  ...
  const prefetchNextPage = useCallback((nextPage: number) => {
    const nextPageParams: RolePermissionParams = {
      ...params,
      page: nextPage
    };

    queryClient.prefetchQuery({
      queryKey: ['rolePermissions', nextPageParams],
      queryFn: async () => {
        try {
          const result = await rolePermissionService.getRolePermissions(nextPageParams);
          return result;
        } catch (error) {
  ...
  const handleFilterApply = useCallback((filters: ActiveFilter[]) => {
    ...
    const newParams: RolePermissionParams = {
      ...params,
      page: 1,
                roleId: 1,
  ...
  const handlePageChange = useCallback((newPage: number) => {
    ...
    localStorage.setItem('rolePermissionsCurrentPage', newPage.toString());
    
    // C蘯ｭp nh蘯ｭt URL v盻嬖 s盻・trang m盻嬖
    navigate(`/dashboard/rolePermissions?page=${newPage}`, { replace: true });
  ...
  const handleChangeRowsPerPage = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Update URL
    navigate('/dashboard/rolePermissions?page=1', { replace: true });
  ...
  // Handle delete rolePermission(s)
  const handleDelete = async (rolePermissionId?: number) => {
    if (rolePermissionId) {
      // Single delete
      setConfirmDialog({
        open: true,
  ...
  const confirmDelete = async () => {
    try {
      if (confirmDialog.multiple) {
        await rolePermissionService.deleteRolePermissions(selected);
        toast.success(t('rolePermissions.deleteMultipleSuccess'));
        setSelected([]);
      } else {
        // Delete single rolePermission
        await rolePermissionService.deleteRolePermission(confirmDialog.rolePermissionId);
        toast.success(t('rolePermissions.deleteSuccess'));
      }
      refetch();
    } catch (error) {
  ...
  // Navigate to create rolePermission page
  const handleCreateRolePermission = () => {
    navigate('/dashboard/rolePermissions/create');
  };

  // Navigate to edit rolePermission page
  const handleEditRolePermission = (rolePermissionId: number) => {
    navigate(`/dashboard/rolePermissions/edit/${rolePermissionId}`);
  };

  // Navigate to view rolePermission page
  const handleViewRolePermission = (rolePermissionId: number) => {
    navigate(`/dashboard/rolePermissions/view/${rolePermissionId}`);
  };
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked && isValidData(data)) {
      const newSelected = data.data.map(rolePermission => rolePermission.id);
  ...
  const renderTableRows = () => {
    ...
    return displayData.map((rolePermission: RolePermission) => {
      const isItemSelected = isSelected(rolePermission.id);
      ...
      return (
        <TableRow 
          ...
          onClick={() => handleSelectOne(rolePermission.id)}
        ...
        >
          <TableCell padding="checkbox">
            <Checkbox
              ...
              onChange={() => handleSelectOne(rolePermission.id)}
          ...
          {columns.map((column) => column.visible && (
            <TableCell key={column.id}>
              {column.id === 'yukoFlag' ? (
                ...
                formatDate(new Date(rolePermission.createdAt).toString())
              ) : column.id === 'id' ? (
                t(`${rolePermission.id}`)) : column.id === 'roleId' ? (
                t(`${rolePermission.roleId}`)) : column.id === 'permissionScreenId' ? (
                t(`${rolePermission.permissionScreenId}`)) : column.id === 'code' ? (
                t(`${rolePermission.code}`)) : column.id === 'updatedAt' ? (
                t(`${rolePermission.updatedAt}`)) : column.id === 'createdUser' ? (
                t(`${rolePermission.createdUser}`)) : column.id === 'lastUpdateUser' ? (
                t(`${rolePermission.lastUpdateUser}`)) : column.id === 'lastUpdateProgram' ? (
                t(`${rolePermission.lastUpdateProgram}`)
              ) : null}
            </TableCell>
          ))}
          <TableCell align="right">
            <Tooltip title={t('common.view')}>
              <span>
                <IconButton 
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleViewRolePermission(rolePermission.id);
                  }}
                >
            ...
            <Tooltip title={!canUpdate ? t('errors.insufficientPermissions') : t('common.edit')}>
              <span>
                <IconButton 
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleEditRolePermission(rolePermission.id);
                  }}
            ...
            <Tooltip title={!canDelete ? t('errors.insufficientPermissions') : t('common.delete')}>
              <span>
                <IconButton 
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleDelete(rolePermission.id);
                  }}
  ...
  const handleFilterClear = () => {
    setActiveFilters([]);
    setFilterValues({});
    setParams((prev: RolePermissionParams) => ({
      ...prev,
                         
      id: undefined,                   
      roleId: undefined,                   
      permissionScreenId: undefined,                   
      code: undefined,                   
      createdAt: undefined,                   
      updatedAt: undefined,                   
      yukoFlag: undefined,                   
  ...
  // Filter options
  const rolePermissionFilterOptions: FilterOption[] = useMemo(() => {
    const baseFilters: FilterOption[] = [      {
        id: 'roleId',
        label: t('rolePermissions.fields.roleId'),
        type: 'text',
      },      {
        id: 'permissionScreenId',
        label: t('rolePermissions.fields.permissionScreenId'),
        type: 'text',
      },      {
        id: 'code',
        label: t('rolePermissions.fields.code'),
        type: 'text',
      },      {
        id: 'createdAt',
        label: t('rolePermissions.fields.createdAt'),
        type: 'text',
      },      {
        id: 'updatedAt',
        ...
        id: 'createdUser',
        label: t('rolePermissions.fields.createdUser'),
        type: 'text',
      },      {
        id: 'lastUpdateUser',
        label: t('rolePermissions.fields.lastUpdateUser'),
        type: 'text',
      },      {
        id: 'lastUpdateProgram',
        label: t('rolePermissions.fields.lastUpdateProgram'),
        type: 'text',
      },
    ];
  ...
  return (
    <Box>
        ...
        >
          ...
          >           
            ...
            >
              ...
              {canDelete && (
                <Tooltip title={selected.length === 0 ? t('rolePermissions.selectToDelete') : t('common.delete')}>
                    <Button
              ...
              <SortMenu 
                ...
                translationPrefix="rolePermissions.fields"
      ...
      <ConfirmDialog
        ...
        title={confirmDialog.multiple ? t('rolePermissions.deleteMultipleTitle') : t('rolePermissions.deleteTitle')}
        message={confirmDialog.multiple 
          ? t('rolePermissions.confirmDeleteMultiple', { count: selected.length }) 
          : t('rolePermissions.confirmDelete')}
      ...
      <Filter
        ...
        defaultFilters={rolePermissionFilterOptions}
      />
