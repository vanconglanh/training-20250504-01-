...
import Filter from '@/pages/DashBoard/PermissionScreen/Filter';
...
import { PermissionScreen, PermissionScreenListResponse, PermissionScreenParams } from '@/types/permissionScreen.type';
...
const STORAGE_KEY = 'permissionScreenTableColumns'; 
const DEFAULT_VISIBLE_COLUMNS = ['username', 'name', 'email', 'roleId', 'yukoFlag']; 

// Column configuration type
interface ColumnConfig {
  id: keyof PermissionScreen | 'actions';
  label: string;
  visible: boolean;
  sortable?: boolean;
}
...
const isValidData = (data: PermissionScreenListResponse | undefined): data is PermissionScreenListResponse => {
...
const PermissionScreenList: React.FC = () => {
  ...
  const getInitialPage = () => {
    ...
    if (pageFromUrl) {
      ...
      localStorage.setItem('permissionScreensCurrentPage', page.toString());
      return page;
    }
    return Number(localStorage.getItem('permissionScreensCurrentPage')) || 1;
  };

  // Pagination and other states
  const [params, setParams] = useState<PermissionScreenParams>({
    page: getInitialPage(),
    size: 10,
    search: [],
  });
  ...
  const [columns, setColumns] = useState<ColumnConfig[]>(() => {
    ...
    return [
      { id: 'id', label: t('permissionScreens.fields.id'), visible: true, sortable: true },
      { id: 'permissionId', label: t('permissionScreens.fields.permissionId'), visible: true, sortable: true },
      { id: 'screenId', label: t('permissionScreens.fields.screenId'), visible: true, sortable: true },
      { id: 'code', label: t('permissionScreens.fields.code'), visible: true, sortable: true },
      { id: 'createdAt', label: t('permissionScreens.fields.createdAt'), visible: true, sortable: true },
      { id: 'updatedAt', label: t('permissionScreens.fields.updatedAt'), visible: true, sortable: true },
      { id: 'yukoFlag', label: t('permissionScreens.fields.yukoFlag'), visible: true, sortable: true },
      { id: 'createdUser', label: t('permissionScreens.fields.createdUser'), visible: true, sortable: true },
      { id: 'lastUpdateUser', label: t('permissionScreens.fields.lastUpdateUser'), visible: true, sortable: true },
      { id: 'lastUpdateProgram', label: t('permissionScreens.fields.lastUpdateProgram'), visible: true, sortable: true },
    ].map(col => ({
      ...col,
  ...
  const [cachedData, setCachedData] = useState<Record<string, PermissionScreen[]>>({});
  const [totalItems, setTotalItems] = useState(0);

  // State ﾄ黛ｻ・lﾆｰu tr盻ｯ d盻ｯ li盻㎡ hi盻ハ th盻・hi盻㌻ t蘯｡i
  const [displayData, setDisplayData] = useState<PermissionScreen[]>([]);
  ...
  const { data, isLoading, isFetching, refetch } = useQuery<PermissionScreenListResponse>({
    queryKey: ['permissionScreens', params],
    queryFn: async () => {
      try {
        const result = await permissionScreenService.getPermissionScreens(params);
        return result;
      } catch (error) {
  ...
  const prefetchNextPage = useCallback((nextPage: number) => {
    const nextPageParams: PermissionScreenParams = {
      ...params,
      page: nextPage
    };

    queryClient.prefetchQuery({
      queryKey: ['permissionScreens', nextPageParams],
      queryFn: async () => {
        try {
          const result = await permissionScreenService.getPermissionScreens(nextPageParams);
          return result;
        } catch (error) {
  ...
  const handleFilterApply = useCallback((filters: ActiveFilter[]) => {
    ...
    const newParams: PermissionScreenParams = {
      ...params,
      page: 1,
                permissionId: 1,
        screenId: 1,
    ...
    filters.forEach(filter => {
      if (filter.field in newParams) {
        const key = filter.field as keyof PermissionScreenParams;
        if (key === 'yukoFlag') {
  ...
  const handlePageChange = useCallback((newPage: number) => {
    ...
    localStorage.setItem('permissionScreensCurrentPage', newPage.toString());
    
    // C蘯ｭp nh蘯ｭt URL v盻嬖 s盻・trang m盻嬖
    navigate(`/dashboard/permissionScreens?page=${newPage}`, { replace: true });
  ...
  const handleChangeRowsPerPage = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Update URL
    navigate('/dashboard/permissionScreens?page=1', { replace: true });
  ...
  // Handle delete permissionScreen(s)
  const handleDelete = async (permissionScreenId?: number) => {
    if (permissionScreenId) {
      // Single delete
      setConfirmDialog({
        open: true,
        permissionScreenId,
  ...
  const confirmDelete = async () => {
    try {
      if (confirmDialog.multiple) {
        await permissionScreenService.deletePermissionScreens(selected);
        toast.success(t('permissionScreens.deleteMultipleSuccess'));
        setSelected([]);
      } else {
        // Delete single permissionScreen
        await permissionScreenService.deletePermissionScreen(confirmDialog.permissionScreenId);
        toast.success(t('permissionScreens.deleteSuccess'));
      }
      refetch();
    } catch (error) {
  ...
  // Navigate to create permissionScreen page
  const handleCreatePermissionScreen = () => {
    navigate('/dashboard/permissionScreens/create');
  };

  // Navigate to edit permissionScreen page
  const handleEditPermissionScreen = (permissionScreenId: number) => {
    navigate(`/dashboard/permissionScreens/edit/${permissionScreenId}`);
  };

  // Navigate to view permissionScreen page
  const handleViewPermissionScreen = (permissionScreenId: number) => {
    navigate(`/dashboard/permissionScreens/view/${permissionScreenId}`);
  };
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked && isValidData(data)) {
      const newSelected = data.data.map(permissionScreen => permissionScreen.id);
  ...
  const renderTableRows = () => {
    ...
    return displayData.map((permissionScreen: PermissionScreen) => {
      const isItemSelected = isSelected(permissionScreen.id);
      
      return (
        <TableRow 
          hover 
          key={permissionScreen.id} 
          sx={{ 
            opacity: isFetching ? 0.7 : 1,
            position: 'relative',
            transition: 'opacity 0.2s ease'
          }}
          onClick={() => handleSelectOne(permissionScreen.id)}
        ...
        >
          <TableCell padding="checkbox">
            <Checkbox
              ...
              onChange={() => handleSelectOne(permissionScreen.id)}
          ...
          {columns.map((column) => column.visible && (
            <TableCell key={column.id}>
              {column.id === 'yukoFlag' ? (
                <Chip 
                  label={t(`statuses.${permissionScreen.yukoFlag ? 'active' : 'inactive'}`)} 
                  color={getStatusColor(Boolean(permissionScreen.yukoFlag))}
                  size="small"
                />
              ) : column.id === 'createdAt' ? (
                formatDate(new Date(permissionScreen.createdAt).toString())
              ) : column.id === 'id' ? (
                t(`${permissionScreen.id}`)) : column.id === 'permissionId' ? (
                t(`${permissionScreen.permissionId}`)) : column.id === 'screenId' ? (
                t(`${permissionScreen.screenId}`)) : column.id === 'code' ? (
                t(`${permissionScreen.code}`)) : column.id === 'updatedAt' ? (
                t(`${permissionScreen.updatedAt}`)) : column.id === 'createdUser' ? (
                t(`${permissionScreen.createdUser}`)) : column.id === 'lastUpdateUser' ? (
                t(`${permissionScreen.lastUpdateUser}`)) : column.id === 'lastUpdateProgram' ? (
                t(`${permissionScreen.lastUpdateProgram}`)
              ) : null}
            </TableCell>
          ))}
          <TableCell align="right">
            <Tooltip title={t('common.view')}>
              <span>
                <IconButton 
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleViewPermissionScreen(permissionScreen.id);
                  }}
                >
            ...
            <Tooltip title={!canUpdate ? t('errors.insufficientPermissions') : t('common.edit')}>
              <span>
                <IconButton 
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleEditPermissionScreen(permissionScreen.id);
                  }}
            ...
            <Tooltip title={!canDelete ? t('errors.insufficientPermissions') : t('common.delete')}>
              <span>
                <IconButton 
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleDelete(permissionScreen.id);
                  }}
  ...
  const handleFilterClear = () => {
    setActiveFilters([]);
    setFilterValues({});
    setParams((prev: PermissionScreenParams) => ({
      ...prev,
  ...
  // Filter options
  const permissionScreenFilterOptions: FilterOption[] = useMemo(() => {
    const baseFilters: FilterOption[] = [      {
        id: 'permissionId',
        label: t('permissionScreens.fields.permissionId'),
        type: 'text',
      },      {
        id: 'screenId',
        label: t('permissionScreens.fields.screenId'),
        type: 'text',
      },      {
        id: 'code',
        label: t('permissionScreens.fields.code'),
        type: 'text',
      },      {
        id: 'createdAt',
        label: t('permissionScreens.fields.createdAt'),
        type: 'text',
      },      {
        id: 'updatedAt',
        label: t('permissionScreens.fields.updatedAt'),
        type: 'text',
      },      {
        id: 'yukoFlag',
        label: t('permissionScreens.fields.yukoFlag'),
        type: 'text',
      },      {
        id: 'createdUser',
        label: t('permissionScreens.fields.createdUser'),
        type: 'text',
      },      {
        id: 'lastUpdateUser',
        label: t('permissionScreens.fields.lastUpdateUser'),
        type: 'text',
      },      {
        id: 'lastUpdateProgram',
        label: t('permissionScreens.fields.lastUpdateProgram'),
        type: 'text',
      },
    ];
    ...
    return baseFilters.filter(filter => {
      ...
      if (['id','permissionId','screenId','code','createdAt','updatedAt','yukoFlag','createdUser','lastUpdateUser','lastUpdateProgram',].includes(filter.id)) {
        return true;
      }
  ...
  return (
    <Box>
        ...
        >
          ...
          >           
            ...
            >
              ...
              {canDelete && (
                <Tooltip title={selected.length === 0 ? t('permissionScreens.selectToDelete') : t('common.delete')}>
                    <Button
              ...
              <SortMenu 
                ...
                translationPrefix="permissionScreens.fields"
              ...
              <Tooltip title={!canCreate ? t('errors.insufficientPermissions') : ''}>
                  <Button 
                    ...
                    onClick={handleCreatePermissionScreen}
                    disabled={!canCreate}
                    fullWidth={isMobile}
                    sx={{ 
                      height: { xs: 36, sm: 42 },
      ...
      <ConfirmDialog
        ...
        title={confirmDialog.multiple ? t('permissionScreens.deleteMultipleTitle') : t('permissionScreens.deleteTitle')}
        message={confirmDialog.multiple 
          ? t('permissionScreens.confirmDeleteMultiple', { count: selected.length }) 
          : t('permissionScreens.confirmDelete')}
      ...
      <Filter
        ...
        defaultFilters={permissionScreenFilterOptions}
      />
