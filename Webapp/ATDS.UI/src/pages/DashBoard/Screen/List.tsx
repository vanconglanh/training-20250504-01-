...
const isValidData = (data: ScreenListResponse | undefined): data is ScreenListResponse => {
...
const ScreenList: React.FC = () => {
  ...
  const getInitialPage = () => {
    ...
    if (pageFromUrl) {
      ...
      localStorage.setItem('screensCurrentPage', page.toString());
      return page;
    }
    return Number(localStorage.getItem('screensCurrentPage')) || 1;
  };
  ...
  const [columns, setColumns] = useState<ColumnConfig[]>(() => {
    ...
    return [
      { id: 'id', label: t('screens.fields.id'), visible: true, sortable: true },
      { id: 'code', label: t('screens.fields.code'), visible: true, sortable: true },
      { id: 'name', label: t('screens.fields.name'), visible: true, sortable: true },
      { id: 'createdAt', label: t('screens.fields.createdAt'), visible: true, sortable: true },
      { id: 'updatedAt', label: t('screens.fields.updatedAt'), visible: true, sortable: true },
      { id: 'yukoFlag', label: t('screens.fields.yukoFlag'), visible: true, sortable: true },
      { id: 'createdUser', label: t('screens.fields.createdUser'), visible: true, sortable: true },
      { id: 'lastUpdateUser', label: t('screens.fields.lastUpdateUser'), visible: true, sortable: true },
      { id: 'lastUpdateProgram', label: t('screens.fields.lastUpdateProgram'), visible: true, sortable: true },
    ].map(col => ({
      ...col,
  ...
  const [cachedData, setCachedData] = useState<Record<string, Screen[]>>({});
  const [totalItems, setTotalItems] = useState(0);

  // State ﾄ黛ｻ・lﾆｰu tr盻ｯ d盻ｯ li盻㎡ hi盻ハ th盻・hi盻㌻ t蘯｡i
  const [displayData, setDisplayData] = useState<Screen[]>([]);
  ...
  const { data, isLoading, isFetching, refetch } = useQuery<ScreenListResponse>({
    queryKey: ['screens', params],
    queryFn: async () => {
      try {
        const result = await screenService.getScreens(params);
        return result;
      } catch (error) {
  ...
  const prefetchNextPage = useCallback((nextPage: number) => {
    const nextPageParams: ScreenParams = {
      ...params,
      page: nextPage
    };

    queryClient.prefetchQuery({
      queryKey: ['screens', nextPageParams],
      queryFn: async () => {
        try {
          const result = await screenService.getScreens(nextPageParams);
          return result;
        } catch (error) {
  ...
  const handleFilterApply = useCallback((filters: ActiveFilter[]) => {
    ...
    filters.forEach(filter => {
      if (filter.field in newParams) {
        const key = filter.field as keyof ScreenParams;
        if (key === 'yukoFlag') {
  ...
  const handlePageChange = useCallback((newPage: number) => {
    ...
    localStorage.setItem('screensCurrentPage', newPage.toString());
    
    // C蘯ｭp nh蘯ｭt URL v盻嬖 s盻・trang m盻嬖
    navigate(`/dashboard/screens?page=${newPage}`, { replace: true });
  ...
  const handleChangeRowsPerPage = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Update URL
    navigate('/dashboard/screens?page=1', { replace: true });
  ...
  const confirmDelete = async () => {
    try {
      if (confirmDialog.multiple) {
        await screenService.deleteScreens(selected);
        toast.success(t('screens.deleteMultipleSuccess'));
        setSelected([]);
      } else {
        // Delete single screen
        await screenService.deleteScreen(confirmDialog.screenId);
        toast.success(t('screens.deleteSuccess'));
      }
      refetch();
    } catch (error) {
  ...
  // Navigate to create screen page
  ...
  const handleEditScreen = (screenId: number) => {
    navigate(`/dashboard/screens/edit/${screenId}`);
  };

  // Navigate to view screen page
  const handleViewScreen = (screenId: number) => {
    navigate(`/dashboard/screens/view/${screenId}`);
  };
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked && isValidData(data)) {
      const newSelected = data.data.map(screen => screen.id);
  ...
  const renderTableRows = () => {
    ...
    return displayData.map((screen: Screen) => {
      const isItemSelected = isSelected(screen.id);
      ...
      return (
        <TableRow 
          ...
          onClick={() => handleSelectOne(screen.id)}
        ...
        >
          <TableCell padding="checkbox">
            <Checkbox
              ...
              onChange={() => handleSelectOne(screen.id)}
          ...
          {columns.map((column) => column.visible && (
            <TableCell key={column.id}>
              {column.id === 'yukoFlag' ? (
                <Chip 
                  label={t(`statuses.${screen.yukoFlag ? 'active' : 'inactive'}`)} 
                  color={getStatusColor(Boolean(screen.yukoFlag))}
                  size="small"
                />
              ) : column.id === 'createdAt' ? (
                formatDate(new Date(screen.createdAt).toString())
              ) : column.id === 'id' ? (
                t(`${screen.id}`)) : column.id === 'code' ? (
                t(`${screen.code}`)) : column.id === 'name' ? (
                t(`${screen.name}`)) : column.id === 'updatedAt' ? (
                t(`${screen.updatedAt}`)) : column.id === 'createdUser' ? (
                t(`${screen.createdUser}`)) : column.id === 'lastUpdateUser' ? (
                t(`${screen.lastUpdateUser}`)) : column.id === 'lastUpdateProgram' ? (
                t(`${screen.lastUpdateProgram}`)
              ) : null}
            </TableCell>
          ))}
          <TableCell align="right">
  ...
  // Filter options
  const screenFilterOptions: FilterOption[] = useMemo(() => {
    const baseFilters: FilterOption[] = [      {
        id: 'code',
        label: t('screens.fields.code'),
        type: 'text',
      },      {
        id: 'name',
        ...
        id: 'createdAt',
        label: t('screens.fields.createdAt'),
        type: 'text',
      },      {
        id: 'updatedAt',
        label: t('screens.fields.updatedAt'),
        type: 'text',
      },      {
        id: 'yukoFlag',
        label: t('screens.fields.yukoFlag'),
        type: 'text',
      },      {
        id: 'createdUser',
        label: t('screens.fields.createdUser'),
        type: 'text',
      },      {
        id: 'lastUpdateUser',
        label: t('screens.fields.lastUpdateUser'),
        type: 'text',
      },      {
        id: 'lastUpdateProgram',
        label: t('screens.fields.lastUpdateProgram'),
        type: 'text',
      },
    ];
  ...
  return (
    <Box>
        ...
        >
          ...
          >           
            ...
            >
              ...
              {canDelete && (
                <Tooltip title={selected.length === 0 ? t('screens.selectToDelete') : t('common.delete')}>
                    <Button
              ...
              <SortMenu 
                ...
                translationPrefix="screens.fields"
      ...
      <ConfirmDialog
        ...
        title={confirmDialog.multiple ? t('screens.deleteMultipleTitle') : t('screens.deleteTitle')}
        message={confirmDialog.multiple 
          ? t('screens.confirmDeleteMultiple', { count: selected.length }) 
          : t('screens.confirmDelete')}
      ...
      <Filter
        ...
        defaultFilters={screenFilterOptions}
      />
