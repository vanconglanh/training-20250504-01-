...
import Filter from '@/pages/DashBoard/Role/Filter';
import { roleService } from '@/services/role.service.ts';
import { ActiveFilter, FilterOption, SortField } from '@/types/common.type';
import { Role, RoleListResponse, RoleParams } from '@/types/role.type';
...
const isValidData = (data: RoleListResponse | undefined): data is RoleListResponse => {
...
const RoleList: React.FC = () => {
  ...
  const getInitialPage = () => {
    ...
    if (pageFromUrl) {
      ...
      localStorage.setItem('rolesCurrentPage', page.toString());
      return page;
    }
    return Number(localStorage.getItem('rolesCurrentPage')) || 1;
  };
  ...
  const [columns, setColumns] = useState<ColumnConfig[]>(() => {
    ...
    return [
      { id: 'id', label: t('roles.fields.id'), visible: true, sortable: true },
      { id: 'code', label: t('roles.fields.code'), visible: true, sortable: true },
      { id: 'name', label: t('roles.fields.name'), visible: true, sortable: true },
  ...
  const [cachedData, setCachedData] = useState<Record<string, Role[]>>({});
  const [totalItems, setTotalItems] = useState(0);

  // State ﾄ黛ｻ・lﾆｰu tr盻ｯ d盻ｯ li盻㎡ hi盻ハ th盻・hi盻㌻ t蘯｡i
  const [displayData, setDisplayData] = useState<Role[]>([]);
  ...
  const { data, isLoading, isFetching, refetch } = useQuery<RoleListResponse>({
  ...
  const prefetchNextPage = useCallback((nextPage: number) => {
    const nextPageParams: RoleParams = {
      ...params,
      page: nextPage
    };

    queryClient.prefetchQuery({
      queryKey: ['roles', nextPageParams],
      queryFn: async () => {
        try {
          const result = await roleService.getRoles(nextPageParams);
          return result;
        } catch (error) {
  ...
  const handlePageChange = useCallback((newPage: number) => {
    ...
    localStorage.setItem('rolesCurrentPage', newPage.toString());
    
    // C蘯ｭp nh蘯ｭt URL v盻嬖 s盻・trang m盻嬖
    navigate(`/dashboard/roles?page=${newPage}`, { replace: true });
  ...
  const handleChangeRowsPerPage = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Update URL
    navigate('/dashboard/roles?page=1', { replace: true });
  ...
  const confirmDelete = async () => {
    try {
      if (confirmDialog.multiple) {
        await roleService.deleteRoles(selected);
        toast.success(t('roles.deleteMultipleSuccess'));
        setSelected([]);
      } else {
        // Delete single role
        await roleService.deleteRole(confirmDialog.roleId);
  ...
  const handleViewRole = (roleId: number) => {
    navigate(`/dashboard/roles/view/${roleId}`);
  };
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked && isValidData(data)) {
      const newSelected = data.data.map(role => role.id);
  ...
  const renderTableRows = () => {
    ...
    return displayData.map((role: Role) => {
      ...
      return (
        <TableRow 
          ...
          onClick={() => handleSelectOne(role.id)}
          ...
          {columns.map((column) => column.visible && (
            <TableCell key={column.id}>
              {column.id === 'yukoFlag' ? (
                ...
                formatDate(new Date(role.createdAt).toString())
              ) : column.id === 'id' ? (
                t(`${role.id}`)) : column.id === 'code' ? (
                t(`${role.code}`)) : column.id === 'name' ? (
                t(`${role.name}`)) : column.id === 'isSystem' ? (
                t(`${role.isSystem}`)) : column.id === 'updatedAt' ? (
                t(`${role.updatedAt}`)) : column.id === 'createdUser' ? (
                t(`${role.createdUser}`)) : column.id === 'lastUpdateUser' ? (
                t(`${role.lastUpdateUser}`)) : column.id === 'lastUpdateProgram' ? (
                t(`${role.lastUpdateProgram}`)
              ) : null}
            </TableCell>
          ))}
          <TableCell align="right">
  ...
  const roleFilterOptions: FilterOption[] = useMemo(() => {
    const baseFilters: FilterOption[] = [      {
        ...
        id: 'isSystem',
        label: t('roles.fields.isSystem'),
        type: 'text',
      },      {
        id: 'createdAt',
        label: t('roles.fields.createdAt'),
        type: 'text',
      },      {
        id: 'updatedAt',
        ...
        id: 'createdUser',
        label: t('roles.fields.createdUser'),
        type: 'text',
      },      {
        id: 'lastUpdateUser',
        label: t('roles.fields.lastUpdateUser'),
        type: 'text',
      },      {
        id: 'lastUpdateProgram',
        label: t('roles.fields.lastUpdateProgram'),
        type: 'text',
      },
    ];
  ...
  return (
    <Box>
        ...
        >
          ...
          >           
            ...
            >
              ...
              {canDelete && (
                <Tooltip title={selected.length === 0 ? t('roles.selectToDelete') : t('common.delete')}>
                    <Button
              ...
              <SortMenu 
                ...
                translationPrefix="roles.fields"
      ...
      <ConfirmDialog
        ...
        title={confirmDialog.multiple ? t('roles.deleteMultipleTitle') : t('roles.deleteTitle')}
        message={confirmDialog.multiple 
          ? t('roles.confirmDeleteMultiple', { count: selected.length }) 
      ...
      <Filter
        ...
        defaultFilters={roleFilterOptions}
      />
