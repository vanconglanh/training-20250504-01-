...
import Filter from '@/pages/DashBoard/VPermission/Filter';
...
import { VPermission, VPermissionListResponse, VPermissionParams } from '@/types/vPermission.type';
...
const isValidData = (data: VPermissionListResponse | undefined): data is VPermissionListResponse => {
...
const VPermissionList: React.FC = () => {
  ...
  const getInitialPage = () => {
    ...
    if (pageFromUrl) {
      ...
      localStorage.setItem('vPermissionsCurrentPage', page.toString());
      return page;
    }
    return Number(localStorage.getItem('vPermissionsCurrentPage')) || 1;
  };

  // Pagination and other states
  const [params, setParams] = useState<VPermissionParams>({
    page: getInitialPage(),
    size: 10,
    search: [],
  });
  ...
  const [columns, setColumns] = useState<ColumnConfig[]>(() => {
    ...
    return [
      { id: 'roleId', label: t('vPermissions.fields.roleId'), visible: true, sortable: true },
      { id: 'roleCode', label: t('vPermissions.fields.roleCode'), visible: true, sortable: true },
      { id: 'roleName', label: t('vPermissions.fields.roleName'), visible: true, sortable: true },
      { id: 'screenId', label: t('vPermissions.fields.screenId'), visible: true, sortable: true },
      { id: 'screenCode', label: t('vPermissions.fields.screenCode'), visible: true, sortable: true },
      { id: 'screenName', label: t('vPermissions.fields.screenName'), visible: true, sortable: true },
      { id: 'permissionId', label: t('vPermissions.fields.permissionId'), visible: true, sortable: true },
      { id: 'permissionCode', label: t('vPermissions.fields.permissionCode'), visible: true, sortable: true },
      { id: 'permissionName', label: t('vPermissions.fields.permissionName'), visible: true, sortable: true },
    ].map(col => ({
      ...col,
  ...
  const [cachedData, setCachedData] = useState<Record<string, VPermission[]>>({});
  const [totalItems, setTotalItems] = useState(0);

  // State ﾄ黛ｻ・lﾆｰu tr盻ｯ d盻ｯ li盻㎡ hi盻ハ th盻・hi盻㌻ t蘯｡i
  const [displayData, setDisplayData] = useState<VPermission[]>([]);
  ...
  const { data, isLoading, isFetching, refetch } = useQuery<VPermissionListResponse>({
    queryKey: ['vPermissions', params],
    queryFn: async () => {
      try {
        const result = await vPermissionService.getVPermissions(params);
        return result;
      } catch (error) {
  ...
  const prefetchNextPage = useCallback((nextPage: number) => {
    const nextPageParams: VPermissionParams = {
      ...params,
      page: nextPage
    };

    queryClient.prefetchQuery({
      queryKey: ['vPermissions', nextPageParams],
      queryFn: async () => {
        try {
          const result = await vPermissionService.getVPermissions(nextPageParams);
          return result;
        } catch (error) {
  ...
  const handleFilterApply = useCallback((filters: ActiveFilter[]) => {
    ...
    const newParams: VPermissionParams = {
      ...params,
      page: 1,
                roleId: 1,
        roleCode: '',
  ...
  const handlePageChange = useCallback((newPage: number) => {
    ...
    localStorage.setItem('vPermissionsCurrentPage', newPage.toString());
    
    // C蘯ｭp nh蘯ｭt URL v盻嬖 s盻・trang m盻嬖
    navigate(`/dashboard/vPermissions?page=${newPage}`, { replace: true });
  ...
  const handleChangeRowsPerPage = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Update URL
    navigate('/dashboard/vPermissions?page=1', { replace: true });
  ...
  // Handle delete vPermission(s)
  const handleDelete = async (vPermissionId?: number) => {
    if (vPermissionId) {
      // Single delete
      setConfirmDialog({
        open: true,
  ...
  const confirmDelete = async () => {
    try {
      if (confirmDialog.multiple) {
        await vPermissionService.deleteVPermissions(selected);
        toast.success(t('vPermissions.deleteMultipleSuccess'));
        setSelected([]);
      } else {
        // Delete single vPermission
        await vPermissionService.deleteVPermission(confirmDialog.vPermissionId);
        toast.success(t('vPermissions.deleteSuccess'));
      }
      refetch();
    } catch (error) {
  ...
  // Navigate to create vPermission page
  const handleCreateVPermission = () => {
    navigate('/dashboard/vPermissions/create');
  };

  // Navigate to edit vPermission page
  const handleEditVPermission = (vPermissionId: number) => {
    navigate(`/dashboard/vPermissions/edit/${vPermissionId}`);
  };

  // Navigate to view vPermission page
  const handleViewVPermission = (vPermissionId: number) => {
    navigate(`/dashboard/vPermissions/view/${vPermissionId}`);
  };
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked && isValidData(data)) {
      const newSelected = data.data.map(vPermission => vPermission.id);
  ...
  const renderTableRows = () => {
    ...
    return displayData.map((vPermission: VPermission) => {
      const isItemSelected = isSelected(vPermission.id);
      ...
      return (
        <TableRow 
          ...
          onClick={() => handleSelectOne(vPermission.id)}
        ...
        >
          <TableCell padding="checkbox">
            <Checkbox
              ...
              onChange={() => handleSelectOne(vPermission.id)}
          ...
          {columns.map((column) => column.visible && (
            <TableCell key={column.id}>
              {column.id === 'yukoFlag' ? (
                ...
                formatDate(new Date(vPermission.createdAt).toString())
              ) : column.id === 'roleId' ? (
                t(`${vPermission.roleId}`)) : column.id === 'roleCode' ? (
                t(`${vPermission.roleCode}`)) : column.id === 'roleName' ? (
                t(`${vPermission.roleName}`)) : column.id === 'screenId' ? (
                t(`${vPermission.screenId}`)) : column.id === 'screenCode' ? (
                t(`${vPermission.screenCode}`)) : column.id === 'screenName' ? (
                t(`${vPermission.screenName}`)) : column.id === 'permissionId' ? (
                t(`${vPermission.permissionId}`)) : column.id === 'permissionCode' ? (
                t(`${vPermission.permissionCode}`)) : column.id === 'permissionName' ? (
                t(`${vPermission.permissionName}`)
              ) : null}
            </TableCell>
          ))}
          <TableCell align="right">
            <Tooltip title={t('common.view')}>
              <span>
                <IconButton 
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleViewVPermission(vPermission.id);
                  }}
                >
            ...
            <Tooltip title={!canUpdate ? t('errors.insufficientPermissions') : t('common.edit')}>
              <span>
                <IconButton 
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleEditVPermission(vPermission.id);
                  }}
  ...
  const handleFilterClear = () => {
    setActiveFilters([]);
    setFilterValues({});
    setParams((prev: VPermissionParams) => ({
      ...prev,
  ...
  // Filter options
  const vPermissionFilterOptions: FilterOption[] = useMemo(() => {
    const baseFilters: FilterOption[] = [      {
        id: 'roleCode',
        label: t('vPermissions.fields.roleCode'),
        type: 'text',
      },      {
        id: 'roleName',
        label: t('vPermissions.fields.roleName'),
        type: 'text',
      },      {
        id: 'screenId',
        label: t('vPermissions.fields.screenId'),
        type: 'text',
      },      {
        id: 'screenCode',
        label: t('vPermissions.fields.screenCode'),
        type: 'text',
      },      {
        id: 'screenName',
        label: t('vPermissions.fields.screenName'),
        type: 'text',
      },      {
        id: 'permissionId',
        label: t('vPermissions.fields.permissionId'),
        type: 'text',
      },      {
        id: 'permissionCode',
        label: t('vPermissions.fields.permissionCode'),
        type: 'text',
      },      {
        id: 'permissionName',
        label: t('vPermissions.fields.permissionName'),
        type: 'text',
      },
    ];
    ...
    return baseFilters.filter(filter => {
      ...
      if (['roleId','roleCode','roleName','screenId','screenCode','screenName','permissionId','permissionCode','permissionName',].includes(filter.id)) {
        return true;
      }
  ...
  return (
    <Box>
        ...
        >
          ...
          >           
            ...
            >
              ...
              {canDelete && (
                <Tooltip title={selected.length === 0 ? t('vPermissions.selectToDelete') : t('common.delete')}>
                    <Button
              ...
              <SortMenu 
                ...
                translationPrefix="vPermissions.fields"
              ...
              <Tooltip title={!canCreate ? t('errors.insufficientPermissions') : ''}>
                  <Button 
                    ...
                    onClick={handleCreateVPermission}
                    disabled={!canCreate}
                    fullWidth={isMobile}
                    sx={{ 
                      height: { xs: 36, sm: 42 },
      ...
      <ConfirmDialog
        ...
        title={confirmDialog.multiple ? t('vPermissions.deleteMultipleTitle') : t('vPermissions.deleteTitle')}
        message={confirmDialog.multiple 
          ? t('vPermissions.confirmDeleteMultiple', { count: selected.length }) 
          : t('vPermissions.confirmDelete')}
      ...
      <Filter
        ...
        defaultFilters={vPermissionFilterOptions}
      />
...
export default VPermissionList;
