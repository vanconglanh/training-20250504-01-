...
import { roleEditSchema, roleSchema } from '@/utils/validation/role.scheme';
...
const roleFields: FormField<RoleFormValues>[] = [
    {
    name: 'name',
    label: 'common.name',
    type: 'text',
    ...
    placeholder: 'roles.placeholders.name'
  },
  {
    name: 'isSystem',
    label: 'common.isSystem',
    type: 'text',
    required: true,
    icon: <PersonIcon color="action" />,
...
const FormFieldRenderer: React.FC<{
  field: FormField<RoleFormValues>;
...
const UpdateOrInsertRole: React.FC = () => {
  const { t } = useTranslation();
  ...
  const currentPage = Number(localStorage.getItem('rolesCurrentPage')) || 1;
  ...
  useEffect(() => {
    ...
    const handlePopState = () => {
      navigate(`/dashboard/roles?page=${currentPage}`, {
        replace: true,
  ...
  const { 
    ...
    isEditMode ? roleEditSchema : roleSchema,
    {
      defaultValues: {
                name: '',
        isSystem: 1,
  ...
  // Query to fetch role data in edit mode
  ...
  // Mutation for create/update role
  const mutation = useMutation({
    mutationFn: async (data: RoleFormValues) => {
      if (isEditMode) {
        return roleService.updateRole(Number(id!), data);
      } else {
        return roleService.createRole(data);
      }
    },
    onSuccess: () => {
      // Invalidate and refetch roles query with exact page
      queryClient.invalidateQueries({ 
        queryKey: ['roles'],
        refetchType: 'all'
      });
      toast.success(t(isEditMode ? 'roles.updateSuccess' : 'roles.createSuccess'));
    ...
    onError: (error) => {
      toast.error(t(isEditMode ? 'roles.updateError' : 'roles.createError'));
  ...
  // Display loading state while fetching role data
...
export default UpdateOrInsertRole;