...
import Filter from '@/pages/DashBoard/User/Filter';
...
const UserList: React.FC = () => {
  ...
  const [columns, setColumns] = useState<ColumnConfig[]>(() => {
    ...
    return [
      { id: 'id', label: t('users.fields.id'), visible: true, sortable: true },
      { id: 'code', label: t('users.fields.code'), visible: true, sortable: true },
      ...
      { id: 'password', label: t('users.fields.password'), visible: true, sortable: true },
      { id: 'passwordHash', label: t('users.fields.passwordHash'), visible: true, sortable: true },
          ...
          {columns.map((column) => column.visible && (
            <TableCell key={column.id}>
              {column.id === 'yukoFlag' ? (
                ...
                t(`${user.id}`)) : column.id === 'code' ? (
                t(`${user.code}`)) : column.id === 'name' ? (
                t(`${user.name}`)) : column.id === 'email' ? (
                t(`${user.email}`)) : column.id === 'username' ? (
                t(`${user.username}`)) : column.id === 'language' ? (
                t(`${user.language}`)) : column.id === 'password' ? (
                t(`${user.password}`)) : column.id === 'passwordHash' ? (
                t(`${user.passwordHash}`)) : column.id === 'roleId' ? (
                t(`${user.roleId}`)) : column.id === 'updatedAt' ? (
                t(`${user.updatedAt}`)) : column.id === 'createdUser' ? (
                t(`${user.createdUser}`)) : column.id === 'lastUpdateUser' ? (
                t(`${user.lastUpdateUser}`)) : column.id === 'lastUpdateProgram' ? (
                t(`${user.lastUpdateProgram}`)
              ) : null}
            </TableCell>
          ))}
          <TableCell align="right">
  ...
  const userFilterOptions: FilterOption[] = useMemo(() => {
    const baseFilters: FilterOption[] = [      {
        ...
        id: 'passwordHash',
        label: t('users.fields.passwordHash'),
        type: 'text',
      },      {
        id: 'roleId',
        ...
        id: 'createdUser',
        label: t('users.fields.createdUser'),
        type: 'text',
      },      {
        id: 'lastUpdateUser',
        label: t('users.fields.lastUpdateUser'),
        type: 'text',
      },      {
        id: 'lastUpdateProgram',
        label: t('users.fields.lastUpdateProgram'),
        type: 'text',
      },
    ];
