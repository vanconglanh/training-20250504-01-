...
const ScreenList: React.FC = () => {
  ...
  const [columns, setColumns] = useState<ColumnConfig[]>(() => {
    ...
    return [
      ...
      { id: 'status', label: t('screens.fields.status'), visible: true, sortable: true },
        ...
        >
          ...
          {columns.map((column) => column.visible && (
            <TableCell key={column.id}>
              {column.id === 'status' ? (
                <Chip 
                  label={t(`statuses.${screen.status ? 'active' : 'inactive'}`)} 
                  color={getStatusColor(Boolean(screen.status))}
                  size="small"
                />
              ) : column.id === 'createdAt' ? (
