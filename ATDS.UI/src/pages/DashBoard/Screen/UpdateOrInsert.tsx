...
import { ScreenFormValues } from '@/types/screen.type';
import { screenEditSchema, screenSchema } from '@/utils/validation/screen.scheme';
...
const screenFields: FormField<ScreenFormValues>[] = [
    {
    name: 'name',
    label: 'common.name',
    type: 'text',
...
const FormFieldRenderer: React.FC<{
  field: FormField<ScreenFormValues>;
  control: Control<ScreenFormValues>;
  errors: FieldErrors<ScreenFormValues>;
  isViewMode: boolean;
...
const UpdateOrInsertScreen: React.FC = () => {
  const { t } = useTranslation();
  ...
  const currentPage = Number(localStorage.getItem('screensCurrentPage')) || 1;
  ...
  useEffect(() => {
    ...
    const handlePopState = () => {
      navigate(`/dashboard/screens?page=${currentPage}`, {
        replace: true,
  ...
  const { 
    ...
    isEditMode ? screenEditSchema : screenSchema,
    {
      defaultValues: {
                name: '',
        status: 1,
  ...
  // Query to fetch screen data in edit mode
  const { data: screenData, isLoading } = useQuery({
    queryKey: ['screen', id],
    queryFn: async () => {
      if (!isEditMode) return null;
      return await screenService.getScreenById(Number(id!));
    },
    enabled: isEditMode
  });

  // Set form values when screen data is loaded
  useEffect(() => {
    if (screenData) {
      reset({
  ...
  // Mutation for create/update screen
  const mutation = useMutation({
    mutationFn: async (data: ScreenFormValues) => {
      if (isEditMode) {
        return screenService.updateScreen(Number(id!), data);
      } else {
        return screenService.createScreen(data);
      }
    },
    onSuccess: () => {
      // Invalidate and refetch screens query with exact page
      queryClient.invalidateQueries({ 
        queryKey: ['screens'],
        refetchType: 'all'
      });
      toast.success(t(isEditMode ? 'screens.updateSuccess' : 'screens.createSuccess'));
    ...
    onError: (error) => {
      toast.error(t(isEditMode ? 'screens.updateError' : 'screens.createError'));
  ...
  // Display loading state while fetching screen data
...
export default UpdateOrInsertScreen;